#!/usr/bin/env python3
"""
AtlasSimple - Pure-Python Property Management Server
...
def _load_secret_key(data_dir):
    default_marker = "atlas_secret_key_change_in_production"
...
DEFAULT_STORAGE_ROOT = r"D:\Storage" if os.name == "nt" else "/app/storage"
STORAGE_ROOT = Path(os.getenv("STORAGE_ROOT", DEFAULT_STORAGE_ROOT))
UPLOAD_DIR = Path(os.getenv("UPLOAD_DIR", str(STORAGE_ROOT)))
HOST = os.getenv("HOST", "127.0.0.1")
PORT = int(os.getenv("PORT", "5000"))
PUBLIC_BASE_URL = (os.getenv("PUBLIC_BASE_URL") or "").strip().rstrip("/")
DATABASE_PATH = os.getenv("DATABASE_PATH", str(DATA_DIR / "atlas.sqlite"))
...
def _host_without_port(host_value):
    h = _normalize_host_value(host_value)
    if not h:
        return ""
...
def _host_allowed(candidate, allowed_rules):
    cand_raw = _normalize_host_value(candidate)
    cand = _host_without_port(cand_raw)
    if not cand_raw or not cand:
        return False
    rules = tuple(allowed_rules or ())
    if not rules:
        return True
    for rule in rules:
        r = _normalize_host_value(rule)
        if not r:
            continue
        if r == "*":
            return True
        rule_host = _host_without_port(r)
        if not rule_host:
            continue
        if rule_host.startswith("."):
            # Suffix wildcard: ".onrender.com" allows "app.onrender.com".
            if cand.endswith(rule_host):
                return True
            continue
        if cand_raw == r or cand == rule_host:
            return True
    return False


def _first_concrete_host(hosts):
    for h in hosts or ():
        h2 = _normalize_host_value(h)
        if h2 and h2 != "*" and not h2.startswith("."):
            return h2
    return ""


ALLOWED_HOSTS = _build_allowed_hosts()
DEFAULT_PUBLIC_HOST = _normalize_host_value(PUBLIC_BASE_URL) or _first_concrete_host(ALLOWED_HOSTS) or "localhost"
...
LOG_FILE = os.getenv("LOG_FILE", str(LOG_DIR / "atlas.log"))
...
REDIS_SESSION_PREFIX = (os.getenv("REDIS_SESSION_PREFIX", "atlas:session:") or "atlas:session:").strip() or "atlas:session:"
REDIS_SESSION_USER_PREFIX = (os.getenv("REDIS_SESSION_USER_PREFIX", "atlas:session_uid:") or "atlas:session_uid:").strip() or "atlas:session_uid:"
SCHEMA_VERSION = 17
...
BOOTSTRAP_ADMIN_FULL_NAME = (os.getenv("BOOTSTRAP_ADMIN_FULL_NAME") or "Atlas Administrator").strip() or "Atlas Administrator"
...
def log_exception(message, **fields):
    ...
    _send_error_alert_once(
        ...
        f"Atlas Critical Error: {fields.get('scope') or 'runtime'}",
...
<div class="public-header"><div><h2>Admin Console</h2><p class="muted">Full control over Atlas.</p></div><a class="ghost-btn" href="/">Home</a></div>
<div class="grid2">
...
<h1 class="door-heading">Atlas Property Management</h1>
...
<a class="ghost-btn" href="/about">About Atlas</a>
</div>
</div>
</div>
</div></section>''',
...
<h2>About Atlas</h2>
<p class="muted">In myth Atlas bears the heavens. In business Atlas bears the weight between landlord and tenant&#8212;payments, documentation, maintenance, and property checks&#8212;so both sides can move with confidence.</p>
</div></div></section>''',
...
<div><b>Email:</b> support@atlas.example</div>
...
<div class="row">
<div class="field" style="flex:1;"><label>Property Name</label><input name="name" required placeholder="Atlas Sunset Apartments"></div>
...
var COMPARE_KEY="atlas_compare_ids";
...
def seed(c):
    if not c.execute("SELECT 1 FROM users LIMIT 1").fetchone():
        ...
        if BOOTSTRAP_ADMIN_USERNAME and BOOTSTRAP_ADMIN_EMAIL and BOOTSTRAP_ADMIN_PHONE and not pw_errs:
            ...
            log_event(logging.WARNING, "demo_seed_blocked", detail="Demo seed data is disabled in hardened builds.")
        else:
    ...
    while pending:
        ...
        for stmt in pending:
            try:
                ...
                msg = str(e).lower()
                if ("does not exist" in msg and ("relation" in msg or "table" in msg) and passes < 8):
                    remaining.append(stmt)
                    continue
                raise RuntimeError(f"postgres_schema_stmt_failed: {stmt}") from e
        if not remaining:
            return
        if not progressed:
    ...
    if SMTP_HOST and SMTP_FROM and email_ok:
        try:
            ...
            if row and (row["email"] or "").strip():
                subj = "Atlas Notification"
                body = f"{text}\n\nOpen Atlas to view details."
                if link:
                    body += f"\nPath: {link}"
...
def send_email(to_email, subject, body_text):
    ...
    try:
        msg = EmailMessage()
        msg["Subject"] = str(subject or "Atlas Notification")[:200]
        msg["From"] = SMTP_FROM
        msg["To"] = to_addr
...
def create_tenant_property_invite(c, sender_user, tenant_ident, property_id, unit_label, message="", owner_account=None):
    ...
    sender_name = sender_user.get("full_name") or sender_user.get("username") or "Atlas"
...
def safe_request_host(headers=None):
    ...
    if not _HOST_RE.fullmatch(raw or ""):
        return fallback
    if not _host_allowed(raw, ALLOWED_HOSTS):
        return fallback
    return raw
...
def main():
    ...
    setup_logging()
    log_event(logging.INFO, "atlas_starting", host=HOST, port=PORT, prod_mode=PROD_MODE)
    bootstrap_files()
    ensure_db()
